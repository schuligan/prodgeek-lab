...
- **Status (2026-06-19): v1 COMPLETE & LIVE.** Done: all sections · scroll-tint · hero aurora + global cursor glow · Bullpen game (charge/aim/3D-arc/fanfare/miss-aww/booking) · motion polish (reveals + Lenis) · Vercel deploy · CI gate · **SEO + security headers (PRO-18)** · **Lighthouse/a11y/responsive/keyboard pass — mobile A11y/BP/SEO/Agentic=100 (PRO-19)** · **booking wired → cal.com/prateek-jha (PRO-17)** · React removed (unused). **Remaining (all post-v1 / optional):** real-browser QA via BrowserStack (PRO-32, backlog), v2 game leaderboard + Supabase (PRO-27/28), delta-checker autonomy agent (PRO-30), autonomy switch-on. **Prateek-only:** real `CALCOM_API_KEY` (for programmatic booking reads only — site works without it), optional custom domain (Vercel → Domains), optional analytics.
